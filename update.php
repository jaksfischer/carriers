<?php
	...
	if ( !empty($_POST))
		...
		if (empty($sobrenome))
                ...
                if (empty($idade))
                {
                    $idade = 'Por favor digite o idade!';
                    $validacao = false;
		}

                if (empty($sobrenome))
                {
                    $sobrenome = 'Por favor preenche o campo!';
                    $validacao = false;
		}
		...
		if ($validacao)
                {
                    ...
                    $sql = "UPDATE funcionarios  set nome = ?, sobrenome = ?, idade = ?, email = ?, sexo = ? WHERE id = ?";
                    $q = $pdo->prepare($sql);
                    $q->execute(array($nome,$sobrenome,$idade,$email,$sexo,$id));
                    Banco::desconectar();
                    header("Location: index.php");
		}
	}
            ...
            <div class="span10 offset1">
                ...
                <form class="form-horizontal" action="update.php?id=<?php echo $id?>" method="post">
                    ...
                    <div class="control-group <?php echo !empty($sobrenomeErro)?'error':'';?>">
                        ...
                        <div class="controls">
                            <input name="sobrenome" class="form-control" size="80" type="text" placeholder="Sobrenome" value="<?php echo !empty($sobrenome)?$sobrenome:'';?>">
                    ...
                    <div class="control-group <?php echo !empty($idadeErro)?'error':'';?>">
                        ...
                        <div class="controls">
                            <input name="idade" class="form-control" size="30" type="text" placeholder="idade" value="<?php echo !empty($idade)?$idade:'';?>">
